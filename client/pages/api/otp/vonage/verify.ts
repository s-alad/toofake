...
export default async function handler(
  ...
  try {
    ...
    let vonage_body_content = JSON.stringify({
      code: otp,
      vonageRequestId: vonage_request_id,
    });
    let vonage_options = {
    ...
    let firebase_refresh_response = await axios.request(
      firebase_refresh_options,
    );
    ...
    let firebase_expiration =
      Date.now() + firebase_refresh_response.data.expires_in * 1000;
    ...
    let access_expiration =
      Date.now() + access_grant_response.data.expires_in * 1000;
