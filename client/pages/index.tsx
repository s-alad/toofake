...
export default function Home() {
  ...
  async function requestOTPVonage(number: string) {
    ...
    let response = axios
      ...
      .catch((error) => {
        failure(JSON.stringify(error.response.data.error));
        helpme();
      ...
      <div className={s.log}>
        {!requestedOtp ? (
          <div className={s.login}>
            <div className={s.text}>login using your phone number</div>
          ...
          <div className={s.verify}>
            <div className={s.text}>enter the one time passcode</div>
            <div className={s.number}>
              <input
        ...
        <div className={s.messages}>
          ...
          {help != "" ? <div className={s.help}>{help}</div> : <span></span>}
        </div>
      </div>
      <div className={s.info}>
        <p>Posting & commenting have been fixed!</p>
        <p>
          TooFake is currently working (I think??) but needs{" "}
          <span>your help!!!</span>
        </p>
        <p>
          BeReal continues to beef up its security making it much harder to
          reverse engineer. If you are well versed in reverse engineering,
          please check out the{" "}
          <a href="https://github.com/s-alad/toofake">
            <FontAwesomeIcon icon={faGithub} /> github
          </a>{" "}
          and help us keep the befake project working!
        </p>
