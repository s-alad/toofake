  ...
  return (
    <div>
      <div className={s.images}>
        <div className={`${s.img}`}>
          <label htmlFor="file-one-upload" className={s.upload}>
            Choose Back image
          </label>
          <input
            id="file-one-upload"
        ...
        <div className={`${s.img}`}>
          <label htmlFor="file-two-upload" className={s.upload}>
            Choose Front image
          </label>
          <input
            id="file-two-upload"
      ...
      <div className={s.info}>
        *some photos taken on an iphone (.heic) may not work. if there is an
        error try taking a screenshot of the image and uploading that instead.
        <br />
        *you might get a client-side exception if the image is too large. The
        maximum limit currently is 4.5mb for both images combined.
      </div>
      {/* fix this nesting */}
      {failure != "" ? (
        <div className={s.failure}>{failure}</div>
      ) : loading ? (
