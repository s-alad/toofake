...
export default function Instant({ instance, mymojis }: _Instant) {
  ...
  async function getLocation() {
    ...
    try {
      ...
      /* console.log(response.data) */
      setLocation(
        response.data.address.Address + ", " + response.data.address.City,
      );
    } catch (error) {
      console.log(error);
      setLocation("No location data");
  ...
  function getReactionState(): JSX.Element {
    if (reactionSuccess) {
      ...
      return (
        <FontAwesomeIcon
          icon={!addingmoji ? faFaceSmile : faFaceFrown}
          onClick={() => setAddingmoji(!addingmoji)}
        />
      );
    }
  }
  ...
  return (
    <div className={s.instant}>
      <div className={s.top}>
        ...
        <div className={s.details}>
          <div className={s.username}>
            <Link href={profile_link}> @{instance.user.username} </Link>
          </div>
        ...
        {instance.user.uid == localStorage.getItem("uid") ? (
          ...
          <div className={s.trash}>
            {reactionLoading ? (
              <div className={s.addloading}>
                <div className={l.loadersmall}></div>
              </div>
            ) : (
              getReactionState()
            )}
      ...
      <div className={s.content}>
        <img
          src={swap ? instance.primary : instance.secondary}
          className={s.primary}
        ...
        >
          <Draggable axis="both" bounds="parent">
            <img
              src={swap ? instance.secondary : instance.primary}
          ...
          <div className={s.realmojis}>
            ...
            {mymojis != undefined
              ? mymojis.map((emoji) => {
                  return (
                    <div
                      className={s.addmojis}
                      key={emoji.id}
                      onClick={() => reactionHandler(emoji)}
                    >
